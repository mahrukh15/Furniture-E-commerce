...
export default function CheckoutButton({ products, totalPrice }: CheckoutButtonProps) {
    const handleCheckout = async () => {
        try {
            const response = await fetch(`http://localhost:3000/api/v4/checkout`, {
                method: "POST",
                headers: {
            ...
            // Ensure localStorage is accessible (client-side)
            const accessToken = typeof window !== "undefined" ? localStorage.getItem("accessToken") : null;

            const orderResponse = await fetch(`http://localhost:3000/api/v4/checkout`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
